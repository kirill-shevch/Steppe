...
Console.Error.WriteLine(
    $"100-year qualification: {options.Years} years, {options.Size}×{options.Size}, seed {options.Seed}, {options.StepMinutes}-minute step");
...
var stem = $"stress-{options.Years}y-{options.Size}x{options.Size}-seed{options.Seed}";
...
static async Task WriteAnnualCsv(string path, IReadOnlyList<AnnualCheckpoint> annual)
{
    ...
    await writer.WriteLineAsync("year,stored_water_mm_per_cell,input_mm_per_cell,output_mm_per_cell,water_error,nitrogen_gm2_per_cell,nitrogen_error,temperature_c,surface_water_mm,root_water_mm,groundwater_mm,snow_mm,live_biomass_gm2,dry_biomass_gm2,available_nitrogen_gm2,plant_nitrogen_gm2,organic_nitrogen_gm2,soil_depth_m,dust_gm2,non_finite");
